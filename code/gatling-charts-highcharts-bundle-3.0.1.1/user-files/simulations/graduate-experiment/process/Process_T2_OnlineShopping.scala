...
object Process_T2_OnlineShopping {
    ...
    var workflow = feed(feeder1)
        ...
        .exec(http("t2-completeWorkitem")   //pay task
            .post("completeTask/${processDefinitionId}/${processInstanceId}/${taskId}")
            .headers(contentType))
        .exec(http("t2-getAllocatedWorkitem")
			.get("getCurrentSingleTask/${processInstanceId}")
			.check(jsonPath("$..taskId").saveAs("taskId")))
        .pause(2) //模拟外部服务的任务处理时间
        .exec(http("t2-completeWorkitem")    //send goods
            .post("completeTask/${processDefinitionId}/${processInstanceId}/${taskId}")
            .headers(contentType))
       .exec(http("t2-getAllocatedWorkitem")
			.get("getCurrentSingleTask/${processInstanceId}")
			.check(jsonPath("$..taskId").saveAs("taskId")))
        .pause(1) //模拟外部服务的任务处理时间
        .exec(http("t2-getAllocatedWorkitem")  //receive goods 
            .post("completeTask/${processDefinitionId}/${processInstanceId}/${taskId}")
            .headers(contentType))
}
